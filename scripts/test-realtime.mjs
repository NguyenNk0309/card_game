...
try {
  ...
  const observerDiscardStatePromise = second.waitForNext((state) => state.game?.outcome?.kind === "discard");
  ...
  assert.equal(observerDiscardState.game.outcome.cardName, undefined, "the realtime observer cannot see the discarded card identity");
  assert.equal(observerDiscardState.game.outcome.cardId, undefined, "the realtime observer cannot see the discarded card ID");
  assert.equal(observerDiscardState.game.history.at(-1).cardName, undefined, "realtime history hides discarded card identity from observers");
  assert(!observerDiscardState.game.history.at(-1).message.includes("Test Skill"), "realtime history does not leak a discarded card name in its message");
  first.send({ type: "return:lobby" });
