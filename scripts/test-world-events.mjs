...

{
  ...
  assert.deepEqual(worldEventEngine.getEligibleTributeCardIds(borrower, state), [sharedCardId, ownedOther], "Tribute reserves only the borrowed same-ID occurrence");
...

{
  ...
  allyState.drawPile = [playerCard(tributeAlly, "common-b").id, playerCard(tributeAlly, "common-c").id];
  allyState.borrowedCards = [{ cardId: borrowedId, ownerId: tributeEnemy.id, borrowedAtTurn: 3 }];
  const enemyChoice = playerCard(tributeEnemy, "special-a").id;
  enemyState.hand = [enemyChoice];
  enemyState.drawPile = [playerCard(tributeEnemy, "common-b").id];
  ...
  assert.deepEqual(worldEventEngine.getEligibleTributeCardIds(tributeAlly, allyState), [allyCommon, allyUnique], "borrowed hand cards are not eligible tribute choices");
  ...
  const otherPlayersCard = worldEventEngine.submitWorldEventChoice(game, players, tributeAlly.id, "tribute-event", [allyCommon, enemyChoice], eventOptions());
  ...
  const firstSubmission = worldEventEngine.submitWorldEventChoice(game, players, tributeAlly.id, "tribute-event", [allyCommon, allyUnique], eventOptions({ now: 30_000, randomInt: minimumRandomInt }));
  ...
  assert(allyState.graveyard.includes(allyUnique), "a unique character card is also a valid tribute");
  assert.equal(allyState.hand.length, 3, "replacement cards restore the vacated hand positions when available");
  ...
  const duplicateSubmit = worldEventEngine.submitWorldEventChoice(game, players, tributeAlly.id, "tribute-event", [allyCommon, allyUnique], eventOptions());
  ...
  const finalSubmission = worldEventEngine.submitWorldEventChoice(game, players, tributeEnemy.id, "tribute-event", [enemyChoice], eventOptions({ now: 35_000, randomInt: minimumRandomInt }));
  ...
  assert.equal(game.completedTurns, turnsBefore);
  assert.equal(game.completedPhases, phasesBefore);
  assert(enemyState.graveyard.includes(enemyChoice));
  ...
  assert.deepEqual(allyOwnResult.privateCardIds, [allyCommon, allyUnique]);
  assert.match(allyOwnResult.privateSummary, /Common A.*Special A/);
  ...
  assert.deepEqual(enemyOwnResult.privateCardIds, [enemyChoice]);
