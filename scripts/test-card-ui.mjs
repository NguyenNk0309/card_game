...
assert.match(styles, /One consistent divider between the four right-sidebar sections[\s\S]*\.rival-panel > :is\(\.turn-queue-card, \.rivalry\)[\s\S]*border-bottom:\s*0;[\s\S]*\.rival-panel > :is\(\.rivalry, \.battle-history-button, \.shop-open-button\)[\s\S]*margin-top:\s*36px;[\s\S]*\.rival-panel > :is\(\.rivalry, \.battle-history-button, \.shop-open-button\)::before[\s\S]*top:\s*-19px;[\s\S]*height:\s*1px;[\s\S]*background:\s*var\(--line\);/, "Turn Order, Team Status, Battle History, and Shop must have one consistent separator between each section");
