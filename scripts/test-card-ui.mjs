...
assert.match(gameApp, /diceBuff=\{visibleDiceModifier\(displayActiveState\?\.diceBuff[\s\S]*shopDiceBonus=\{visibleDiceModifier\(displayActiveState\?\.shopDiceBonus[\s\S]*markedTargetBonus=\{visibleDiceModifier\(markedTargetDiceBonus/, "ordinary buffs, Shop bonuses, and Marked Target must remain distinct tooltip sources");
...
for (const [token, hex] of Object.entries({ damage: "#e5484d", heal: "#35b96f", shield: "#3b82f6", attack: "#f26a3d", "healing-support": "#84cc5a", dice: "#8b5cf6", marked: "#d946a8", turn: "#f5a524", cards: "#22b8cf", dispel: "#14b8a6", support: "#eabf32", none: "#8b9098", pity: "var(--gold)" })) {
...
assert.match(styles, /\.automatic-success-notice,[\s\S]*\.history-dice\.automatic[\s\S]*color:\s*var\(--effect-pity\)\s*!important;/, "zero-pity automatic-success surfaces must use the approved gold");
...
assert.doesNotMatch(cardFace, /row\.effectTone|failureEffect === "self-damage"/, "card SUCCESS and FAILURE rows must remain independent of semantic effect tones");
assert.match(styles, /\.gothic-card-result-row\.success \.gothic-card-result-icon\s*\{[^}]*color:\s*var\(--pixel-text-green\)\s*!important;[^}]*border-color:\s*var\(--pixel-text-green\);[^}]*\}[\s\S]*\.gothic-card-result-row\.failure \.gothic-card-result-icon\s*\{[^}]*color:\s*var\(--pixel-text-red\)\s*!important;[^}]*border-color:\s*var\(--pixel-text-red\);[^}]*filter:\s*none;[^}]*\}[\s\S]*\.gothic-card-result-row\.success > b\s*\{\s*color:\s*var\(--pixel-text-green\)\s*!important;\s*\}[\s\S]*\.gothic-card-result-row\.failure > b\s*\{\s*color:\s*var\(--pixel-text-red\)\s*!important;\s*\}/, "card result icons and labels must always use fixed green SUCCESS and red FAILURE colors");
assert.match(styles, /\.card-hover-tooltip-results > section\.success > span,[\s\S]*color:\s*var\(--pixel-text-green\);[\s\S]*\.card-hover-tooltip-results > section\.failure > span,[\s\S]*color:\s*var\(--pixel-text-red\);/, "card preview result icons and labels must keep the same fixed state colors");
...
assert.match(styles, /Shared panel surfaces[\s\S]*\.turn-queue-card,[\s\S]*\.rivalry,[\s\S]*box-shadow:\s*var\(--pixel-shadow\), var\(--pixel-inset\);[\s\S]*One consistent divider between the four right-sidebar sections[\s\S]*\.rival-panel > :is\(\.turn-queue-card, \.rivalry\)[\s\S]*padding:\s*12px;[\s\S]*border:\s*2px solid #55483d;[\s\S]*\.rival-panel > :is\(\.rivalry, \.battle-history-button, \.shop-open-button\)[\s\S]*margin-top:\s*48px;[\s\S]*\.rival-panel > :is\(\.rivalry, \.battle-history-button, \.shop-open-button\)::before[\s\S]*top:\s*-25px;[\s\S]*height:\s*2px;[\s\S]*background:\s*rgba\(212, 181, 110, \.45\);/, "Turn Order and Team Status must use their own app-native boxes while all four sidebar sections retain strong, evenly spaced separators");
assert.doesNotMatch(styles, /\.rival-panel > \.rivalry\s*\{[^}]*margin-top:/, "Team Status must use the same centered divider gap as History and Shop");
...
assert.match(sharedShop, /function appendShopUseNotice[\s\S]*kind: 'shop-use'[\s\S]*actorId: player\.id[\s\S]*shopOfferId: offer\.id[\s\S]*if \(offer\.category === 'potion'\) \{[\s\S]*appendShopUseHistory\(game, player, offer, now\);[\s\S]*appendShopUseNotice\(game, player, offer, now\);[\s\S]*export function useShopItem[\s\S]*appendShopUseHistory\(game, player, offer, now\);[\s\S]*appendShopUseNotice\(game, player, offer, now\)/, "Potions record and toast on immediate activation while Items do so only when used");
...
assert.match(gameApp, /function GameNoticeTitle[\s\S]*formatViewpointText\(notice\.title, players, notice\.actorId \?\? localPlayer\?\.id, \{ useActualNames: true \}\)[\s\S]*actor\.displayName[\s\S]*shop-notice-offer[\s\S]*offer\.name[\s\S]*<GameNoticeTitle notice=\{notice\}[\s\S]*formatViewpointText\(notice\.detail, players, notice\.actorId \?\? localPlayer\?\.id, \{ useActualNames: true \}\)[^>]*useActualNames/, "toast titles and details must replace second-person wording with the acting player's real name while preserving Shop offer colors");
assert.match(gameApp, /function ConfirmedTopAction[\s\S]*whileHover=\{\{ y: -2 \}\}[\s\S]*whileTap=\{subtleTap\}/, "topbar confirmation buttons must lift without scaling wide enough to create a horizontal scrollbar");
...
assert.match(styles, /Shared panel surfaces[\s\S]*\.team-block,[\s\S]*box-shadow:\s*var\(--pixel-shadow\), var\(--pixel-inset\);[\s\S]*Each roster team is its own app-native panel[\s\S]*\.party-rail \.team-block\s*\{[^}]*padding:\s*10px;[^}]*border:\s*2px solid #55483d;/, "Veilbound and Embercourt must each use their own app-native roster box");
...
assert.match(gameApp, /className="panel-expand-button shop-open-button"[\s\S]*className="shop-open-title"><ShoppingBag size=\{17\}\/?> SHOP[\s\S]*className="shop-open-gold"><Coins size=\{16\}\/?> \{formatGoldUnits\(displayLocalState\.goldUnits \?\? 0\)\} GOLD/, "the Shop launcher must center a History-sized title above a larger available-Gold line");
...
assert.match(gameApp, /const \[presentedGame, setPresentedGame\][\s\S]*const automaticPanelVisible = Boolean\(activeAutoPanel \|\| showPendingWorldEventChoice\)[\s\S]*automaticPanelSequenceRef\.current = true[\s\S]*const settleAutomaticPanelExit[\s\S]*if \(automaticPanelVisibleRef\.current\) return;[\s\S]*setPresentedGame\(latestGameRef\.current\)/, "gameplay impacts must retain the previous presented snapshot throughout the full automatic-panel sequence");
assert.match(gameApp, /<AnimatePresence onExitComplete=\{settleAutomaticPanelExit\}>\{showPendingWorldEventChoice[\s\S]*<PartyRail players=\{players\} game=\{displayGame\}[\s\S]*<BattleHand[\s\S]*cards=\{displayedLocalHand\}[\s\S]*<AnimatePresence initial=\{false\} mode="wait" onExitComplete=\{settleAutomaticPanelExit\}>/, "pending World Event and standard panel exits must share the release boundary for roster, status, and hand impacts");
