...
assert.match(styles, /One consistent divider between the four right-sidebar sections[\s\S]*\.rival-panel > :is\(\.turn-queue-card, \.rivalry\)[\s\S]*border-bottom:\s*0;[\s\S]*\.rival-panel > :is\(\.rivalry, \.battle-history-button, \.shop-open-button\)::before[\s\S]*top:\s*-19px;[\s\S]*height:\s*1px;[\s\S]*background:\s*var\(--line\);/, "Turn Order, Team Status, Battle History, and Shop must have one consistent separator between each section");
assert.match(styles, /\.rival-panel > \.rivalry\s*\{\s*margin-top:\s*19px;/, "the first sidebar divider must sit directly below Turn Order without a redundant empty strip");
