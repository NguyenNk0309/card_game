...
assert.match(styles, /\.dice-modifier-value\s*\{[^}]*color:\s*#e2b4ff;[^}]*border:\s*1px solid #9e5bc4;[\s\S]*\.dice-modifier-anchor:hover \.dice-modifier-tooltip,[\s\S]*\.dice-modifier-anchor:focus-within \.dice-modifier-tooltip/, "the purple boxed modifier must reveal its detail tooltip on hover or keyboard focus");
assert.match(sharedTypes, /cardBonus\?: number;[\s\S]*passiveBonus\?: number;[\s\S]*shopDiceBonus\?: number;[\s\S]*markedTargetBonus\?: number;/, "card outcomes must retain each consumed modifier source for result presentation");
...
assert.match(gameApp, /function OutcomeRollEquation[\s\S]*from .* card[\s\S]*passiveName.*passive[\s\S]*from d20 buff[\s\S]*from Truecast Tonic potion[\s\S]*from Marked Target card[\s\S]*from debuff[\s\S]*Raw d20 result[\s\S]*total modifier[\s\S]*Meet or exceed/, "the local action result must describe every equation value and each active modifier source");
assert.match(gameApp, /showOutcome && outcome && outcomePresentation \?[\s\S]*<OutcomeRollEquation outcome=\{outcome\}[\s\S]*className="modal-lead resolution-outcome-detail"/, "the acting player's action result must render detailed roll math and larger outcome copy");
const observerActionOutcomeLine = gameApp.split(/\r?\n/).find((line) => line.includes("showActionOutcome && outcome && outcomePresentation")) ?? "";
assert(observerActionOutcomeLine.includes('className="modal-lead resolution-outcome-detail"') && !observerActionOutcomeLine.includes("OutcomeRollEquation"), "other-player Action Outcome must enlarge its copy without exposing private modifier values");
assert.match(styles, /\.resolution-card\s*\{[^}]*container-type:\s*inline-size;[\s\S]*\.resolution-equation\s*\{[^}]*display:\s*grid;[^}]*grid-template-columns:[^}]*minmax\(170px, 1\.6fr\)[\s\S]*\.resolution-equation \.resolution-value-detail\s*\{[^}]*font-size:\s*clamp\(10px, 1\.25vw, 12px\)[\s\S]*\.resolution-outcome-detail\s*\{[^}]*font-size:\s*clamp\(16px, 2\.3vw, 19px\)[\s\S]*@container \(max-width: 640px\)[\s\S]*grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\)/, "detailed Action Outcome values must grow responsively without overflowing the modal");
