...
assert.match(styles, /@media \(min-width: 1280px\) and \(max-width: 1439px\)[\s\S]*\.dice-panel\s*\{[^}]*grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\);[^}]*grid-template-rows:\s*auto minmax\(58px, auto\) minmax\(52px, auto\);[\s\S]*\.dice-panel \.dice-action-check\s*\{[^}]*grid-column:\s*1 \/ -1;[\s\S]*\.dice-panel \.turn-action-buttons\s*\{[^}]*grid-column:\s*1 \/ -1;[^}]*grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\);/, "sub-1440 desktop controls must place Action Check first and the four buttons below it in equal two-button rows");
...
assert.match(gameApp, /function OutcomeEffectSummary[\s\S]*!breakdown\.id\.startsWith\("gold-"\)[\s\S]*withoutAutomaticGold[\s\S]*breakdown\.parts[\s\S]*outcome-effect-number[\s\S]*<OutcomeEffectSummary outcome=\{outcome\}/, "card Action Results must render concise effect math while hiding automatic Gold gain");
...
assert.match(styles, /\.outcome-effect-summary\s*\{[^}]*width:\s*min\(620px, 100%\);[^}]*text-align:\s*center;[\s\S]*\.outcome-effect-summary p\s*\{[^}]*font-size:\s*clamp\(15px, 2vw, 18px\);[\s\S]*\.outcome-effect-number\s*\{[^}]*color:\s*#ddb0f5;[^}]*font-weight:\s*900;/, "effect sentences must remain responsive and highlight every value in purple");
