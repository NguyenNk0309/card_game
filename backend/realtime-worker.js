...
function applyWorldEvent(game, turn, now) {
  const level = Math.ceil(turn / 5); const team = Math.random() < 0.5 ? 'veil' : 'ember'; const living = (filterTeam) => room.players.filter((player) => (!filterTeam || player.hero.team === filterTeam) && (game.playerStates[player.id]?.hp || 0) > 0); const kind = Math.floor(Math.random() * 5); let title = 'Địa chấn chiến trường'; let description = '';
  if (kind === 0) { for (const player of living()) { const reduction = living(player.hero.team).some((ally) => ally.hero.classId === 'oracle') ? 1 : 0; game.playerStates[player.id].hp = Math.max(0, game.playerStates[player.id].hp - Math.max(0, level - reduction)); } description = `Mọi người còn sống nhận ${level} sát thương; đội có Tiên tri giảm 1.`; }
  else if (kind === 1) { title = 'Tiếp tế khẩn cấp'; for (const player of living(team)) game.playerStates[player.id].hp = Math.min(game.playerStates[player.id].maxHp, game.playerStates[player.id].hp + level); description = `${teamLabel(team)} hồi ${level} HP cho mọi thành viên còn sống.`; }
  else if (kind === 2) { title = 'Sóng phá giáp'; for (const state of Object.values(game.playerStates)) state.shield = Math.max(0, (state.shield || 0) - level * 2); description = `Mọi người mất tối đa ${level * 2} khiên.`; }
  else if (kind === 3) { title = 'Khí thế cuồng nộ'; for (const player of living(team)) game.playerStates[player.id].attackBuff = (game.playerStates[player.id].attackBuff || 0) + level; description = `${teamLabel(team)} nhận +${level} sát thương cho đòn đánh kế tiếp.`; }
  else { title = 'Mưa tên vô chủ'; const reduction = living(team).some((ally) => ally.hero.classId === 'oracle') ? 1 : 0; for (const player of living(team)) game.playerStates[player.id].hp = Math.max(0, game.playerStates[player.id].hp - Math.max(0, level + 1 - reduction)); description = `${teamLabel(team)} nhận ${level + 1} sát thương bất ngờ; Tiên tri giảm 1.`; }
  const event = { id: `world-${turn}-${now}`, turn, level, title, description, affectedTeam: kind === 0 || kind === 2 ? undefined : team }; game.worldEvent = event; game.history.push({ id: `${event.id}-history`, turn, kind: 'world', actorName: 'Sự kiện thế giới', message: `${title}: ${description}`, success: true, createdAt: now });
}
...
async function applyCommand(ownerId, message) {
  ...
  if (message.type === 'game:update') {
    ...
    if ((room.game.playerStates[activePlayer.id]?.hp || 0) <= 0) return 'Người chơi đã gục không thể đánh bài.';
    room.game = message.game;
    normalizeServerTurnOrder(room.game);
    await commitRoom();
    return null;
