...
function reconcilePityPoints(previousGame, incomingGame, actor) {
  ...
  if (outcome.resolution === 'pity') {
    ...
    if (freePityActive || activeContext.testMode) outcome.success = true;
...
async function passCurrentTurn(kind, now = Date.now(), discardedCardName = '', discardedCardId = '') {
  ...
  if (revived.length) game.history.push({ id: `revive-${completedTurns}-${now}`, turn: completedTurns, phase: actionPhase, kind: 'buff', actorName: revived.map((player) => player.displayName).join(', '), actorTeam: revived[0]?.hero.team, targetName: revived.map((player) => player.displayName).join(', '), eventName: 'Immediate Resurrection', message: `${revived.map((player) => player.displayName).join(', ')} revived with one-third HP.`, success: true, createdAt: now });
  game.roll = null;
  let phaseCompleted = false;
  if (passingPlayer) {
...
async function applyCommand(ownerId, message, deadlineAdvanced = false) {
  ...
  if (message.type === 'shop:buy' || message.type === 'shop:exchange-pity' || message.type === 'shop:use-item') {
    if (room.phase !== 'game' || !room.game) return 'There is no active battle Shop.';
    if (!ownerId || ownerId !== message.sessionId) return 'You can only use your own Shop account.';
    const result = message.type === 'shop:buy'
      ? purchaseShopOffer(room.game, room.players, ownerId, String(message.offerId || ''))
      : message.type === 'shop:exchange-pity'
        ? exchangePityForGold(room.game, room.players, ownerId)
        : useShopItem(room.game, room.players, ownerId, String(message.itemId || ''));
    if (!result.ok) return result.error || 'The Shop action failed.';
    await commitRoom();
    return null;
  }
