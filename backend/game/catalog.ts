...
export const HERO_TEMPLATES: Omit<Hero, "id" | "team" | "isYou">[] = [
  ...
  { name: "Brother Orren", title: "The Burden Bearer", role: "Healer", classId: "healer", className: "Restoration Cleric", passiveName: "Enduring Grace", passiveText: "Orren's Heal cards restore 2 additional HP to the chosen ally.", skill: "Prayer of Life", skillText: "Restore a large amount of HP to an ally in danger.", summary: "A dedicated healer who restores wounded allies and can immediately return one defeated ally.", strength: "Highest direct healing and the only ally revival.", weakness: "No special damage, and Returning Light can be used only once per battle.", impact: "Can reverse one defeat and immediately return that ally to the current phase.", hp: 10, maxHp: 10, speed: 4, color: "#789bad", initials: "BO" },
  { name: "Nyx Calder", title: "The Blade in Shadow", role: "Assassin", classId: "assassin", className: "Saboteur Assassin", passiveName: "Armor Pierce", passiveText: "Every Nyx attack ignores all shield.", skill: "Quiet Knife", skillText: "Strike directly through an enemy's shield.", summary: "A shield-piercing finisher who can temporarily steal an enemy hand card, preferring special cards.", strength: "Ignores shield and disrupts an enemy's strongest hand options.", weakness: "Low HP and high backlash on the strongest strike.", impact: "Punishes shield-heavy teams and borrows a priority enemy option until Nyx's next turn ends.", hp: 8, maxHp: 8, speed: 10, color: "#9a83b7", initials: "NC" },
  ...
  { name: "Sable Fen", title: "The Eye of the Storm", role: "Controller", classId: "oracle", className: "Fate Oracle", passiveName: "Second Sight", passiveText: "The first time Sable is defeated, she immediately revives with half of her maximum HP.", skill: "Favorable Omen", skillText: "Make one ally's next played card cost 0 pity.", summary: "A fate controller who grants one ally a zero-pity card, curses enemy rolls, cancels one enemy turn, and can return from defeat once.", strength: "Powerful pity and tempo control with a one-time self-revival.", weakness: "Low HP, no special damage, and severe team backlash.", impact: "Makes one crucial allied card automatic, changes who gets to act, then bends fate once to survive defeat.", hp: 8, maxHp: 8, speed: 7, color: "#6aa8a5", initials: "SF" },
  ...
  { name: "Ione Mire", title: "The Oathkeeper", role: "Support", classId: "support", className: "Tactical Commander", passiveName: "Commanding Voice", passiveText: "Ione gains +1 to every d20 result.", skill: "Attack Order", skillText: "Increase the next attack damage of the whole team.", summary: "A tactical support commander who buffs allies and temporarily purges a random card from an enemy hand.", strength: "Reliable d20 results, team-wide preparation, and temporary card denial.", weakness: "No special damage, shield, or healing.", impact: "Makes every teammate better while briefly removing an enemy option from circulation.", hp: 9, maxHp: 9, speed: 6, color: "#bd9f76", initials: "IM" },
...
export const CHARACTER_SKILL_CARDS: Record<string, CharacterSkillCard[]> = {
  ...
  "Mira Ash": [
    ...
    { id: "ma-gravity", name: "Gravity Hex", type: "Wit", description: "One living enemy suffers -2 to d20 rolls. Expires at the end of the target's next turn.", bonus: 0, effect: "support", target: "enemy", value: 2, supportType: "enemy-dice", failureEffect: "team-damage", failureValue: 1 }
  ],
  "Brother Orren": [
    ...
    { id: "bo-return", name: "Returning Light", type: "Spirit", description: "Choose a defeated ally. Revive them immediately with one-third HP; they take the next turn after Brother Orren in the current phase. After its first use, this card moves to the graveyard.", bonus: 0, effect: "support", target: "defeated-ally", value: 1, supportType: "revive", failureEffect: "team-damage", failureValue: 2 }
  ],
  "Nyx Calder": [
    ...
    { id: "nc-pilfer", name: "Pilfered Chance", type: "Wit", description: "Choose one living enemy. Steal 1 random card from their hand, preferring special cards when available. Return it to that enemy's discard pile when Nyx's next turn ends.", bonus: 0, effect: "support", target: "enemy", value: 1, supportType: "steal-card", failureEffect: "self-damage", failureValue: 2 }
  ],
  "Bram Coalhand": [
  ...
  "Sable Fen": [
    { id: "sf-favor", name: "Favorable Omen", type: "Spirit", description: "Choose one living ally, including yourself. The next card they play during their next turn has 0 pity cost. If they do not play a card, this effect expires at the end of that turn. After its third use, Favorable Omen moves to Sable Fen's graveyard.", bonus: 0, effect: "support", target: "ally", value: 2, supportType: "zero-pity", failureEffect: "self-damage", failureValue: 1 },
    { id: "sf-hex", name: "Dark Omen", type: "Wit", description: "One living enemy suffers -3 to d20 rolls. Expires at the end of the target's next turn.", bonus: 0, effect: "support", target: "enemy", value: 3, supportType: "enemy-dice", failureEffect: "team-damage", failureValue: 1 },
  ...
  "Ione Mire": [
    ...
    { id: "im-focus", name: "Focus Order", type: "Spirit", description: "All living allies, including yourself, gain +2 to d20 rolls. Expires at the end of each target's next turn.", bonus: 0, effect: "support", target: "all-allies", value: 2, supportType: "dice", failureEffect: "self-damage", failureValue: 1 },
    { id: "im-purge", name: "Tactical Purge", type: "Wit", description: "Choose one living enemy. Move one random card from their hand to their graveyard for 2 phases. After 2 phases, return that card to their discard pile. After its third use, Tactical Purge also moves to Ione's graveyard.", bonus: 0, effect: "support", target: "enemy", value: 2, supportType: "purge-card", failureEffect: "self-damage", failureValue: 2 }
  ],
  "Dagan Flint": [
