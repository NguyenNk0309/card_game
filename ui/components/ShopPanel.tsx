  ...
  return <m.div className="shop-panel-content" variants={fadePresence} initial="hidden" animate="visible" exit="exit">
    ...
    <section className="shop-exchange-bar">
      ...
      <div className="shop-exchange-actions">
        <div className="shop-wallet"><Coins size={22}/><span><small>YOUR GOLD</small><AnimatePresence initial={false} mode="popLayout"><m.strong key={gold} variants={popPresence} initial="hidden" animate="visible" exit="exit">{gold} / {MAX_GOLD}</m.strong></AnimatePresence></span></div>
    ...
    <LayoutGroup id="shop-tabs"><nav className="shop-tabs" aria-label="Shop categories">{tabs.map(({ id, label, icon: Icon }) => <m.button className={tab === id ? "active" : ""} aria-pressed={tab === id} onClick={() => setTab(id)} whileTap={subtleTap} key={id}>{tab === id && <m.i className="shop-tab-active" layoutId="shop-active-tab" transition={motionTransition.layout}/>}<Icon size={17}/><span>{label}</span>{id === "inventory" && <b>{inventorySize}/{SHOP_INVENTORY_CAP}</b>}</m.button>)}</nav></LayoutGroup>
