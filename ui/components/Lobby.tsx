      ...
      <div className="lobby-team-slots">
        {members.map((player, index) => <article className={`joined-player team-slot-player ${selected?.id === player.id ? "selected" : ""}`} key={player.id}>
          <button className="joined-main" onClick={() => onSelectPlayer(player.id)}>
            ...
            <div><div className="joined-name"><strong className={`player-name-highlight ${relationClass(player)}`}>{playerDisplayName(player, localSessionId)}</strong>{player.id === localSessionId && <span>Your session</span>}</div><p>{player.hero.name} · {player.hero.className}</p></div>
          ...
          <div className="joined-actions"><button onClick={() => onSelectPlayer(player.id)}><Eye size={14}/> Review deck</button>{player.id === localSessionId ? <><button className={player.ready ? "unready-button" : "ready-button"} onClick={() => onToggleReady(player.id)}>{player.ready ? "Cancel ready" : "Ready"}</button><button className="leave-button" onClick={() => onLeave(player.id)} aria-label="Leave room as YOU" title="Leave room"><UserMinus size={15}/></button></> : localPlayer ? <button className="remove-player-button" onClick={() => onRemovePlayer(player.id)}><UserMinus size={14}/> Remove</button> : <span className="remote-player-label">Another browser</span>}</div>
        </article>)}
  ...
  return <section className="lobby-stage">
    ...
    <div className="lobby-grid">
      <section className="join-panel">
        <div className="lobby-panel-heading"><div><span className="eyebrow">SHARED GAME ROOM</span><h2>{localPlayer ? `YOU joined ${localPlayer.hero.team === "veil" ? "Veilbound" : "Embercourt"}` : "Enter your name and choose a slot"}</h2></div><div className="lobby-heading-status"><span className={`connection-pill ${connectionStatus}`}>{statusText[connectionStatus] ?? connectionStatus}</span><div className={players.length >= 10 ? "capacity full" : "capacity"}><Users size={16} /> {players.length}/10</div></div></div>
      ...
      </> : selectedHeroName === RANDOM_CHARACTER_SELECTION ? <div className="no-character random-character-preview"><Sparkles size={32}/><h2>Random character</h2><p>Fate will choose one of the available characters when you claim an empty team slot.</p><strong>Your chosen team is preserved.</strong></div> : <div className="no-character"><Sparkles size={28}/><h2>Your character awaits</h2><p>Choose a character to review their special skills.</p></div>}</aside>
    </div>
  </section>;
}
