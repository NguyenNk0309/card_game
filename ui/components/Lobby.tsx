...
export function Lobby({ players, playerName, error, selectedPlayerId, localSessionId, connectionStatus, characterOptions, selectedHeroName, onNameChange, onJoin, onSelectPlayer, onToggleReady, onLeave, onRemovePlayer, onEnterGame, onHeroSelect }: Props) {
  ...
  return <section className="lobby-stage">
    <div className="lobby-intro"><span className="eyebrow">THE ARENA AWAITS</span><h1>Join the battle.</h1><p>Choose one of 10 character classes, review an 8-card deck with 3 special cards and 5 common cards, then ready up with your team.</p></div>
    <div className="lobby-grid">
      ...
      <aside className="character-panel">{shownHero ? <>
        ...
        <div className="character-stats"><span><strong>{shownHero.hp}</strong> HP</span><span><strong>8</strong> cards · 3 special</span><span><strong>{shownHero.skill}</strong> Signature</span></div>
        ...
        <div className="lobby-skill-deck">{shownDeck.map((card) => <article className={`skill-card ${card.unique ? "hero-unique-card" : "common-skill-card"}`} key={card.id} style={{ "--hero-color": shownHero.color } as React.CSSProperties}>{card.unique && <div className="unique-card-banner"><Crown size={13}/> SPECIAL · {shownHero.name}</div>}<div className={`card-sigil ${card.type.toLowerCase()}`}>{card.type === "Might" ? <Swords size={18}/> : card.type === "Wit" ? <Eye size={18}/> : <Sparkles size={18}/>}</div><span className="skill-kind">{card.unique ? "Class skill" : "Common card"} · {card.type} · +{card.bonus}</span><strong>{card.name}</strong><p>{card.description}</p><small>{getCardEffectLabel(card)} · {getCardTargetLabel(card)}</small><p className="card-impact">{describeCardImpact(card)}</p></article>)}</div>
