...
type Props = {
  ...
  onEnterGame: () => void; onHeroSelect: (heroName: string) => void; onTeamSelect: (team: TeamId) => void;
};
...
export function Lobby({ players, playerName, error, selectedPlayerId, localSessionId, connectionStatus, characterOptions, selectedHeroName, onNameChange, onJoin, onSelectPlayer, onToggleReady, onLeave, onRemovePlayer, onEnterGame, onHeroSelect, onTeamSelect }: Props) {
  ...
  return <section className="lobby-stage">
    ...
    <div className="lobby-grid">
      <section className="join-panel">
        ...
        <div className="joined-list">
          ...
          {players.map((player, index) => <article className={`joined-player ${selected?.id === player.id ? "selected" : ""}`} key={player.id}>
            <button className="joined-main" onClick={() => onSelectPlayer(player.id)}><div className="portrait" style={{ "--hero-color": player.hero.color } as React.CSSProperties}>{player.hero.initials}</div><div><div className="joined-name"><strong className={`player-name-highlight ${relationClass(player)}`}>{player.displayName}</strong><span>{player.id === localSessionId ? "Your session" : `Session ${index + 1}`}</span></div><p>{player.hero.name} · {player.hero.className}</p></div><div className="lobby-player-status">{player.ready && <span className={`ready-team-badge ${player.hero.team}`}>{player.hero.team === "veil" ? <Eye size={12}/> : <Flame size={12}/>} {player.hero.team === "veil" ? "Veilbound" : "Embercourt"}</span>}<span className={`ready-badge ${player.ready ? "is-ready" : ""}`}>{player.ready && <Check size={13}/>} {player.ready ? "Ready" : "Not ready"}</span></div></button>
            {player.id === localSessionId && <div className="joined-team-choice" role="group" aria-label="Choose your team"><span>{player.ready ? "Team locked while ready" : "Choose your team"}</span><button className={player.hero.team === "veil" ? "selected veil" : "veil"} onClick={() => onTeamSelect("veil")} aria-pressed={player.hero.team === "veil"} disabled={player.ready} title={player.ready ? "Cancel ready before changing teams" : "Choose Veilbound"}><Eye size={14}/> Veilbound</button><button className={player.hero.team === "ember" ? "selected ember" : "ember"} onClick={() => onTeamSelect("ember")} aria-pressed={player.hero.team === "ember"} disabled={player.ready} title={player.ready ? "Cancel ready before changing teams" : "Choose Embercourt"}><Flame size={14}/> Embercourt</button></div>}
        ...
        <div className={`ready-gate ${canStart ? "all-ready" : ""}`}>{canStart ? <Check size={19}/> : <Shield size={19}/>}<div><strong>{canStart ? "Everyone is ready" : players.length < 2 ? "Waiting for another player" : !hasBothTeams ? "Both teams need a player" : "Waiting for all players"}</strong><span>{canStart ? "Any joined player may start the battle." : !hasBothTeams && players.length >= 2 ? "At least one player must choose Veilbound and one must choose Embercourt." : "Every joined player must press Ready before the battle can start."}</span></div><button className="enter-game-button" onClick={onEnterGame} disabled={!canStart || connectionStatus !== "connected"}><Swords size={17}/> Enter the battle</button></div>
      </section>
      <aside className="character-panel">{shownHero ? <>
        ...
        <div className="character-stats"><span className="stat-health"><strong>{shownHero.hp}</strong><span>HP</span></span><span className="stat-speed"><strong>{shownHero.speed}</strong><span>Speed</span></span><span className="stat-cards"><strong>10</strong><span>cards · <b>3</b> special</span></span><span className="stat-signature"><strong>{shownHero.skill}</strong><span>Signature</span></span></div>
