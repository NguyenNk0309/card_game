  ...
  return <section className="lobby-stage">
    ...
    <div className="lobby-grid">
      ...
      <aside className="character-panel">
        <div className={`hero-picker ${characterPickLocked ? "is-locked" : ""}`}><div className="deck-heading"><div><span className="eyebrow">CHOOSE YOUR CHARACTER</span><strong>{characterPickLocked ? "Cancel ready to change your pick" : "Optional · Review before you ready up"}</strong></div><Users size={18}/></div><div className="hero-class-tabs" role="tablist" aria-label="Character classes">{characterGroups.map((group) => <button type="button" role="tab" aria-selected={activeCharacterGroup === group.id} className={activeCharacterGroup === group.id ? "active" : ""} onClick={() => setActiveCharacterGroup(group.id)} key={group.id}>{group.label}<span>{characterOptions.filter((option) => group.roles.includes(option.hero.role)).length}</span></button>)}</div><div className="hero-picker-grid" role="tabpanel" aria-label={`${activeGroup.label} characters`}>{visibleCharacterOptions.map((option) => <button type="button" key={option.hero.name} className={selectedHeroName === option.hero.name ? "selected" : ""} aria-pressed={selectedHeroName === option.hero.name} onClick={() => onHeroSelect(selectedHeroName === option.hero.name ? "" : option.hero.name)} disabled={characterPickLocked} title={characterPickLocked ? "Cancel ready before changing characters." : `${option.hero.summary} Click again to unselect.`}><span style={{ "--hero-color": option.hero.color } as React.CSSProperties}>{option.hero.initials}</span><strong>{option.hero.name}</strong><small>{option.hero.className}</small></button>)}{!visibleCharacterOptions.length && <div className="empty-character-group"><Sparkles size={22}/><span>No characters currently match this class.</span></div>}</div></div>
        {shownHero ? <div className="character-review-layout">
          <section className="character-info-column">
            <div className="character-banner"><div className="large-portrait" style={{ "--hero-color": shownHero.color } as React.CSSProperties}>{shownHero.initials}</div><div><span className="eyebrow">{reviewingOwnPick ? "YOUR CHARACTER PICK" : "SELECTED CHARACTER"}</span><h2>{shownHero.name}</h2><p>{shownHero.title} · {shownHero.className}</p></div></div>
