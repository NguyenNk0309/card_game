...
export function Lobby({ players, playerName, error, selectedPlayerId, localSessionId, connectionStatus, characterOptions, selectedHeroName, onNameChange, onJoin, onSelectPlayer, onToggleReady, onLeave, onRemovePlayer, onEnterGame, onHeroSelect }: Props) {
  ...
  return <section className="lobby-stage">
    <div className="lobby-intro"><span className="eyebrow">THE ARENA AWAITS</span><h1>Join the battle.</h1><p>Choose one of 10 character classes, review a 13-card deck with 3 special cards, 5 action cards, and 5 no-effect cards, then ready up with your team.</p></div>
    <div className="lobby-grid">
      <section className="join-panel">
        ...
        <div className="joined-list">
          ...
          {players.map((player, index) => <article className={`joined-player ${selected?.id === player.id ? "selected" : ""}`} key={player.id}>
            <button className="joined-main" onClick={() => onSelectPlayer(player.id)}><div className="portrait" style={{ "--hero-color": player.hero.color } as React.CSSProperties}>{player.hero.initials}</div><div><div className="joined-name"><strong>{player.displayName}</strong><span>{player.id === localSessionId ? "Your session" : `Session ${index + 1}`}</span></div><p>{player.hero.name} · {player.hero.className}</p></div><span className={`ready-badge ${player.ready ? "is-ready" : ""}`}>{player.ready && <Check size={13}/>} {player.ready ? "Ready" : "Not ready"}</span></button>
      ...
      <aside className="character-panel">{shownHero ? <>
        ...
        <div className="character-stats"><span><strong>{shownHero.hp}</strong> HP</span><span><strong>13</strong> cards · 3 special</span><span><strong>{shownHero.skill}</strong> Signature</span></div>
        ...
        <div className="lobby-skill-deck">{shownDeck.map((card) => <article className={`skill-card ${card.unique ? "hero-unique-card" : "common-skill-card"} ${card.effect === "none" ? "no-effect-card" : ""}`} key={card.id} style={{ "--hero-color": shownHero.color } as React.CSSProperties}>{card.unique && <div className="unique-card-banner"><Crown size={13}/> SPECIAL · {shownHero.name}</div>}<div className={`card-sigil ${card.type.toLowerCase()}`}>{card.type === "Might" ? <Swords size={18}/> : card.type === "Wit" ? <Eye size={18}/> : <Sparkles size={18}/>}</div><span className="skill-kind">{card.unique ? "Class skill" : card.effect === "none" ? "No-effect common card" : "Common action card"} · {card.type}</span><strong>{card.name}</strong><p>{card.description}</p><small>{getCardEffectLabel(card)} · {getCardTargetLabel(card)}</small><div className="card-outcome-lines"><p className="card-success-line"><Check size={13}/><span><b>SUCCESS</b>{describeCardSuccess(card)}</span></p><p className="card-failure-line"><X size={13}/><span><b>FAILURE</b>{describeCardFailure(card)}</span></p></div></article>)}</div>
