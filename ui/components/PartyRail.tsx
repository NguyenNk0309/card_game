        ...
        {members.map((player) => {
          ...
          return <article className={`hero-row ${dead ? "is-dead" : ""} ${player.id === localSessionId ? "is-you" : ""}`} key={player.id}>
            ...
            <div className="hero-copy">
              ...
              <div className="hp-line"><Heart size={11} fill="currentColor"/><div className="hp-meter" role="progressbar" aria-label={`${player.displayName} health`} aria-valuemin={0} aria-valuemax={maxHp} aria-valuenow={hp}><i style={{ width: `${Math.max(0, hp / maxHp) * 100}%` }}/><strong>{hp} / {maxHp} HP</strong></div>{Boolean(state?.shield) && <span className="roster-shield" title={`${state?.shield} shield`}><Shield size={11}/>{state?.shield}</span>}</div>
              {buffs.length > 0 && <div className="roster-buff-row" aria-label={`${player.displayName} active effects`}>{buffs.map((buff, index) => <span className={`roster-buff-indicator ${buff.negative ? "negative" : ""}`} tabIndex={0} aria-label={`${buff.label}: ${buff.value}`} key={`${buff.label}-${index}`}><Sparkles size={12}/><span className="roster-buff-tooltip" role="tooltip"><strong>{buff.label}</strong><span className={buff.negative ? "negative" : ""}><em>{buff.value}</em></span></span></span>)}</div>}
            </div>
          </article>;
        })}
      </section>;
