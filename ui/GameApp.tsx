...
export default function GameApp() {
  ...
  return <main className="game-shell arena-focus"><div className="grain"/>{showBattleVfx && vfxCard && <div className={`battle-card-vfx effect-${vfxCard.effect} ${outcome?.success ? "success" : "failure"}`} aria-hidden="true"><i/><i/><i/><div><CardEffectIcon card={vfxCard}/><strong>{vfxCard.name}</strong></div></div>}{phase === "game" && localPlayer && <button className="discard-card-button" onClick={discardCard} disabled={activePlayer?.id !== sessionId || status !== "connected" || runComplete || rolling || !activeCard || (activeState?.hp ?? 0) <= 0}><Trash2 size={19}/><span>Discard selected card</span><small>Draw replacement · End turn</small></button>}{phase === "game" && <button className="world-event-library-button" onClick={() => setExpandedPanel("events")}><Zap size={15}/><span>All world events</span></button>}{inspectedPlayer && <aside className="inspected-deck-overlay"><PublicDeck player={inspectedPlayer}/></aside>}{showGuide && <DetailedGuide onClose={() => setShowGuide(false)}/>}
