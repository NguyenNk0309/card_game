...
function HandCardContents({ card, player }: { card: ActionCard; player: PlayerSession }) {
  return <>{card.unique && <span className="special-skill-banner"><Crown size={12}/> {player.hero.className.toUpperCase()} SKILL</span>}<div className={`card-sigil effect-${card.effect}`}><CardEffectIcon card={card}/></div><span>{card.unique ? "Special card" : card.effect === "none" ? "No-effect card" : "Common action"} · {typeName[card.type]}</span><strong>{card.name}</strong><p>{card.description}</p><small>{getCardEffectLabel(card)} · {getCardTargetLabel(card)}</small><CardOutcomeLines card={card}/></>;
}

function PublicDeck({ player, localPlayer }: { player: PlayerSession; localPlayer?: PlayerSession }) {
  return <div className="public-character-deck"><div className="public-deck-heading"><div><span className="eyebrow">PUBLIC CHARACTER DECK · <b className={`player-name-highlight ${playerRelationClass(player, localPlayer)}`}>{player.displayName}</b></span><strong>{player.hero.name} · 10 cards</strong></div></div><div>{player.skillDeck.map((card) => <article key={card.id} className={`public-deck-card effect-${card.effect} ${card.unique ? "public-special-card" : ""}`} style={{ "--hero-color": player.hero.color } as React.CSSProperties}>{card.unique && <span className="unique-card-banner"><Crown size={13}/> SPECIAL · {player.hero.name}</span>}<div className={`card-sigil effect-${card.effect}`}><CardEffectIcon card={card}/></div><div><span>{card.unique ? "Special" : "Common"} · {card.type}</span><strong>{card.name}</strong><small>{getCardEffectLabel(card)} · {getCardTargetLabel(card)}</small><p>{card.description}</p><CardOutcomeLines card={card}/></div></article>)}</div></div>;
}

function DetailedGuide({ onClose }: { onClose: () => void }) {
  return <div className="guide-update-backdrop" onClick={onClose}><section className="guide-update-panel" onClick={(event) => event.stopPropagation()}><button className="modal-close icon-button" onClick={onClose}><X size={18}/></button><span className="eyebrow">COMPLETE UPDATED GUIDE</span><h2>How to play Shattered Oath</h2><div className="guide-update-grid"><article><strong>1 · Build your warrior</strong><p>Players may choose the same character. Every public 10-card deck has 3 special cards, 2 common attacks, 1 common shield, 1 common heal, and 3 no-effect cards.</p></article><article><strong>2 · Speed sets each phase</strong><p>Living players act from highest Speed to lowest. The expanded turn branch marks completed turns gray, highlights the active player, and leaves future turns pending. A phase completes after every living player acts, then the branch resets.</p></article><article><strong>3 · Play, discard, or skip</strong><p>Select one of your four private hand cards. Roll to resolve it, discard it to draw a random replacement and end your turn, or skip while preserving your full hand.</p></article><article><strong>4 · Card zones and graveyard</strong><p>Draw and discard retain a visible order, while replacements are randomly selected. An empty draw pile reshuffles discard. Cards moved to the graveyard are permanently excluded from hand, draw, discard, and every reshuffle for the rest of the battle.</p></article><article><strong>5 · Beat the d20 target</strong><p>Targets range from 8 to 16. Active buffs, penalties, and matching passives modify the result. Failed common cards do nothing; failed special cards apply their printed backlash.</p></article><article><strong>6 · Effects and defeat</strong><p>Damage hits shield before HP unless it pierces. Healing cannot revive unless stated. Defeated players are gray and leave turn order. Sable can revive herself once at half HP.</p></article><article><strong>7 · World events</strong><p>A world event triggers every fifth completed phase. Use the event-library button beside the timer to review every possible event and its random effects.</p></article><article><strong>8 · Victory</strong><p>Eliminate the opposing team immediately, or lead on total HP after phase 30. Remaining living players, shield, and influence resolve ties.</p></article></div></section></div>;
}
...
function HistoryEntries({ entries, players, localPlayer, expanded = false }: { entries: GameHistoryEntry[]; players: PlayerSession[]; localPlayer?: PlayerSession; expanded?: boolean }) {
  ...
  return <div className={`history-list ${expanded ? "expanded-history-list" : ""}`}>{!visibleEntries.length && <p className="empty-history">No actions yet.</p>}{[...visibleEntries].reverse().map((entry) => <article className={`history-entry ${entry.kind} ${entry.success ? "success" : "failure"}`} key={entry.id}><span className={entry.kind === "world" ? "history-event-label" : ""}>{entry.kind === "world" && <Zap size={12}/>} {entry.kind === "world" ? `WORLD EVENT · PHASE ${entry.phase ?? entry.turn}` : `${expanded ? `Phase ${entry.phase ?? "—"} · ` : ""}Turn ${entry.turn}`}</span><strong className={`history-player-name ${entry.actorTeam ? `player-name-highlight ${teamRelationClass(entry.actorTeam, localPlayer?.hero.team)}` : "world"}`}>{entry.actorName}</strong><p><HighlightPlayerNames text={entry.message} players={players} localPlayer={localPlayer}/></p>{entry.diceRoll != null && <div className="history-dice"><span>d20: {entry.diceRoll}</span><span>Active/passive bonus: +{entry.diceBonus ?? 0}</span>{Boolean(entry.dicePenalty) && <span>Penalty: -{entry.dicePenalty}</span>}<strong>Total {entry.diceTotal} / target {entry.diceTarget}</strong></div>}</article>)}</div>;
}
...
export default function GameApp() {
  ...
  return <main className="game-shell arena-focus"><div className="grain"/>{showBattleVfx && vfxCard && <div className={`battle-card-vfx effect-${vfxCard.effect} ${outcome?.success ? "success" : "failure"}`} aria-hidden="true"><i/><i/><i/><div><CardEffectIcon card={vfxCard}/><strong>{vfxCard.name}</strong></div></div>}{cardZoneMotion && localPlayer && !panelOverlayOpen && <CardZoneVfx key={cardZoneMotion.id} motion={cardZoneMotion} player={localPlayer} playable={activePlayer?.id === sessionId && !runComplete && (localState?.hp ?? 0) > 0}/>} {phase === "game" && localPlayer && <button className="discard-card-button" onClick={discardCard} disabled={activePlayer?.id !== sessionId || status !== "connected" || runComplete || rolling || !activeCard || (activeState?.hp ?? 0) <= 0}><Trash2 size={19}/><span>Discard selected card</span><small>Draw replacement · End turn</small></button>}{showGuide && <DetailedGuide onClose={() => setShowGuide(false)}/>}
    ...
    {phase === "lobby" ? <Lobby players={players} playerName={playerName} error={lobbyError || roomError} selectedPlayerId={selectedPlayerId} localSessionId={sessionId} connectionStatus={status} characterOptions={characterOptions} selectedHeroName={selectedHeroName} onNameChange={(name) => { setPlayerName(name); setLobbyError(""); clearError(); }} onJoin={joinPlayer} onSelectPlayer={setSelectedPlayerId} onToggleReady={toggleReady} onLeave={leaveLobby} onRemovePlayer={removePlayer} onEnterGame={enterGame} onHeroSelect={(name) => { setSelectedHeroName(name); setLobbyError(""); }} onTeamSelect={selectTeam}/> :
    ...
    {modalOpen && <div className="modal-backdrop" role="dialog" aria-modal="true" onClick={closeModal}><section className={`modal-card ${showGuide ? "tutorial-modal" : ""} ${deckReview || expandedPanel || inspectedPlayer ? "wide-modal" : ""} ${(showOutcome || showTurnSummary || showWorldEvent) && !showGuide && !deckReview && !expandedPanel && !inspectedPlayer ? "resolution-card" : ""}`} onClick={(event) => event.stopPropagation()}><button className="modal-close icon-button" onClick={closeModal} aria-label="Close"><X size={18}/></button>
      ...
      deckReview ? <div className="expanded-panel-content"><span className="eyebrow">YOUR PRIVATE DECK</span><h2>{deckReview === "draw" ? "Draw pile" : deckReview === "discard" ? "Discard pile" : "Graveyard"}</h2><p className="modal-lead">{deckReview === "draw" ? "These cards remain available to draw. Their order is shown only to you." : deckReview === "discard" ? "Only cycled cards enter this pile. It reshuffles when the draw pile needs a replacement." : "These cards are permanently out of circulation and cannot return to hand, draw pile, or discard pile during this battle."}</p>{reviewedCards.length ? <div className="pile-card-grid">{reviewedCards.map((card, index) => <article className={`pile-review-card effect-${card.effect} ${card.unique ? "special" : ""}`} key={`${card.id}-${index}`}><span>{index + 1} · {card.unique ? "Special" : "Common"} · {card.type}</span><div className={`card-sigil effect-${card.effect}`}><CardEffectIcon card={card}/></div><strong>{card.name}</strong><small>{getCardEffectLabel(card)} · {getCardTargetLabel(card)}</small><p>{card.description}</p><CardOutcomeLines card={card}/></article>)}</div> : <div className="private-hand-empty"><Archive size={24}/><strong>This pile is empty.</strong></div>}</div> :
