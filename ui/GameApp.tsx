...
function RoomGame({ roomId, onRoomUnavailable, onReturnHome }: { roomId: string; onRoomUnavailable: (message: string) => void; onReturnHome: () => void }) {
  ...
  const [presentedGame, setPresentedGame] = useState<SyncedGameState | null>(() => room.game ?? null);
  ...
  const latestGameRef = useRef<SyncedGameState | null>(room.game ?? null);
  const automaticPanelVisibleRef = useRef(false);
  const automaticPanelSequenceRef = useRef(false);
  ...
  const automaticPanelVisible = Boolean(activeAutoPanel || showPendingWorldEventChoice);
  useLayoutEffect(() => {
    latestGameRef.current = game ?? null;
    automaticPanelVisibleRef.current = automaticPanelVisible;
    if (phase !== "game") {
      automaticPanelSequenceRef.current = false;
      setPresentedGame(game ?? null);
    } else if (automaticPanelVisible) automaticPanelSequenceRef.current = true;
    else if (showGuide || manualPanelOpen || !automaticPanelSequenceRef.current) setPresentedGame(game ?? null);
  }, [automaticPanelVisible, game, manualPanelOpen, phase, showGuide]);
  const settleAutomaticPanelExit = useCallback(() => {
    if (automaticPanelVisibleRef.current) return;
    automaticPanelSequenceRef.current = false;
    setPresentedGame(latestGameRef.current);
  }, []);
  const displayGame = presentedGame ?? game;
  const displayQueuedActiveId = displayGame?.turnOrder?.[0];
  const displayActivePlayerIndex = displayQueuedActiveId ? Math.max(0, players.findIndex((player) => player.id === displayQueuedActiveId)) : displayGame?.activePlayerIndex ?? 0;
  const displayActivePlayer = players[displayActivePlayerIndex];
  const displayActiveState = displayActivePlayer ? displayGame?.playerStates[displayActivePlayer.id] : undefined;
  const displayLocalState = localPlayer ? displayGame?.playerStates[localPlayer.id] : undefined;
  const displayedHandKey = displayLocalState?.hand.join("|") ?? "";
  const displayedLocalHand = useMemo(() => (displayLocalState?.hand ?? []).map((id) => cardCatalogById.get(id)).filter((card): card is ActionCard => Boolean(card)), [displayedHandKey, cardCatalogById]);
  const displayLocalPassiveActive = localPlayer && displayLocalState ? isCharacterPassiveActive(localPlayer, displayLocalState) : false;
  ...
  const visibleTurnOrder = useMemo(() => {
    ...
    const living = players.filter((player) => (displayGame.playerStates[player.id]?.hp ?? 0) > 0);
    const ids = displayGame.turnOrder?.length
      ? displayGame.turnOrder
      : [...players.slice(displayActivePlayerIndex), ...players.slice(0, displayActivePlayerIndex)].map((player) => player.id);
    return [...new Set([...ids, ...living.map((player) => player.id)])]
      .map((id) => players.find((player) => player.id === id))
      .filter((player): player is NonNullable<typeof player> => Boolean(player && (displayGame.playerStates[player.id]?.hp ?? 0) > 0));
  }, [displayGame, players, displayActivePlayerIndex]);
  ...
  const teamTotals = (team: TeamId, sourceGame = displayGame) => {
    const members = players.filter((player) => player.hero.team === team);
    return {
      hp: members.reduce((sum, player) => sum + (sourceGame?.playerStates[player.id]?.hp ?? player.hero.hp), 0),
      maxHp: members.reduce((sum, player) => sum + (sourceGame?.playerStates[player.id]?.maxHp ?? player.hero.maxHp), 0),
      shield: members.reduce((sum, player) => sum + (sourceGame?.playerStates[player.id]?.shield ?? 0) + (sourceGame?.playerStates[player.id]?.goldenShield ?? 0), 0),
      alive: members.filter((player) => (sourceGame?.playerStates[player.id]?.hp ?? player.hero.hp) > 0).length,
      total: members.length
    };
  };
  const veil = teamTotals("veil");
  const ember = teamTotals("ember");
  const resultVeil = teamTotals("veil", game);
  const resultEmber = teamTotals("ember", game);
  ...
  return <main className="game-shell arena-focus"><div className="grain"/>
    ...
    <header className="topbar"><div className="brand"><div className="brand-mark"><Crown size={20}/></div><div><strong>SHATTERED OATH</strong><span>Two teams. One victor.</span></div></div>
      {phase === "game" ? <RunStatus completedPhases={displayGame?.completedPhases ?? Math.max(0, (displayGame?.roundNumber ?? 1) - 1)} turnDeadline={game?.turnDeadline} serverTimeOffsetMs={serverTimeOffsetMs} worldEvents={displayGame?.worldEventHistory ?? []} worldEventPlan={displayGame?.worldEventPlan} pendingEvent={pendingWorldEvent} onOpenWorldEvents={() => setExpandedPanel("world-events")}/> : <div className="lobby-top-status"><Users size={16}/> {players.length}/10 players · {players.filter((player) => player.ready).length} ready</div>}
    ...
    <AnimatePresence initial={false} mode="sync">{phase === "lobby" ? <m.div className="room-phase-stage" key="lobby" variants={screenPresence} initial="hidden" animate="visible" exit="exit" transition={motionTransition.screen}><Lobby roomId={roomId} players={players} playerName={playerName} error={lobbyError || roomError} selectedPlayerId={selectedPlayerId} localSessionId={sessionId} connectionStatus={status} characterOptions={characterOptions} selectedHeroName={selectedHeroName} onNameChange={(name) => { setPlayerName(name); window.localStorage.setItem(PLAYER_NAME_STORAGE_KEY, name); setLobbyError(""); clearError(); }} onSlotSelect={selectLobbySlot} onSelectPlayer={setSelectedPlayerId} onToggleReady={toggleReady} onLeave={leaveLobby} onRemovePlayer={removePlayer} onEnterGame={enterGame} onHeroSelect={selectLobbyHero} onReturnHome={onReturnHome}/></m.div> :
      <m.div className="room-phase-stage" key="battle" variants={screenPresence} initial="hidden" animate="visible" exit="exit" transition={motionTransition.screen}><div className="game-layout"><PartyRail players={players} game={displayGame} localSessionId={localPlayer ? sessionId : ""} onInspectPlayer={inspectPlayer}/>
        <section className="world-stage combat-stage"><div className="realm-meta"><div><h1>Eliminate the opposing team</h1></div></div>
          <div className="encounter-row"><TurnStatusBanner isLocalActiveTurn={isLocalActiveTurn} worldEventBlocking={worldEventBlocking} pendingTitle={pendingWorldEvent?.title} activePlayer={displayActivePlayer} players={players} localPlayer={localPlayer}/><DiceRoller rolling={rolling} target={displayedDiceTarget} passiveBonus={visibleDiceModifier(passiveDiceBonus, displayActivePlayer?.id, sessionId)} passiveName={displayActivePlayer?.hero.passiveName} diceBuff={visibleDiceModifier(displayActiveState?.diceBuff, displayActivePlayer?.id, sessionId)} dicePenalty={visibleDiceModifier(displayActiveState?.dicePenalty, displayActivePlayer?.id, sessionId)} shopDiceBonus={visibleDiceModifier(displayActiveState?.shopDiceBonus, displayActivePlayer?.id, sessionId)} markedTargetBonus={visibleDiceModifier(markedTargetDiceBonus, displayActivePlayer?.id, sessionId)} pityPoints={displayLocalState?.pityPoints ?? 0} pityCost={activePityCost} hasSelectedCard={Boolean(activeCard)} canPlaySelectedCard={activeCardCanBePlayed} selectedCardBlockReason={activeCardBlockReason} onRoll={castDie} onPity={usePityRoll} onSkip={skipTurn} onDiscard={discardCard} disabled={worldEventBlocking || activePlayer?.id !== sessionId || status !== "connected" || runComplete || (localState?.hp ?? 1) <= 0}/></div>
          ...
          <section className="hand-zone private-hand-zone"><div className="hand-heading"><div><span className="eyebrow">{localPlayer ? <><span className={`player-name-highlight ${playerRelationClass(localPlayer, localPlayer)}`}>{localPlayer.displayName}</span>&apos;S PRIVATE HAND</> : "PRIVATE HAND"}</span><strong>{displayActivePlayer?.id === sessionId ? "Choose a card and target, then roll. Drag any card to reorder." : "Plan while the current player acts. Drag any card to reorder."}</strong></div>{localPlayer && <div className="pile-review-actions"><span><Hand size={16}/> Hand ({displayLocalState?.hand.length ?? 0})</span><button onClick={() => setDeckReview("draw")}><Layers size={16}/> Draw pile ({displayLocalState?.drawPile.length ?? 0})</button><button onClick={() => setDeckReview("discard")}><Archive size={16}/> Discard ({displayLocalState?.discardPile.length ?? 0})</button><button onClick={() => setDeckReview("graveyard")}><Skull size={16}/> Graveyard ({displayLocalState?.graveyard?.length ?? 0})</button></div>}</div>
            {localPlayer && displayLocalState && <m.div layout className="active-passive-strip"><b>ACTIVE PASSIVE</b><m.span layout className={`active-passive-effect${displayLocalPassiveActive ? " is-active" : ""}`} title={`${localPlayer.hero.passiveName}: ${localPlayer.hero.passiveText}`} aria-label={`${localPlayer.hero.passiveName} passive ${displayLocalPassiveActive ? "active" : "inactive"}`} transition={motionTransition.standard}>{localPlayer.hero.passiveText}</m.span></m.div>}
            {localPlayer ? <BattleHand
              cards={displayedLocalHand}
              selectedCard={selectedCard}
              selectionActive={isLocalActiveTurn}
              playable={isLocalActiveTurn && status === "connected" && !rolling}
              hp={displayLocalState?.hp}
