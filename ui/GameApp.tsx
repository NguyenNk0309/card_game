...
export default function GameApp() {
  ...
  const modalOpen = showGuide || showOutcome || showWorldEvent || runComplete;
  ...
  return <main className="game-shell arena-focus"><div className="grain"/>
    ...
    {modalOpen && <div className="modal-backdrop" role="dialog" aria-modal="true"><section className={`modal-card ${showGuide ? "tutorial-modal" : ""} ${(showOutcome || showWorldEvent) && !showGuide ? "resolution-card" : ""}`}><button className="modal-close icon-button" onClick={closeModal} aria-label="Close"><X size={18}/></button>
      {showGuide ? <div className="tutorial-scroll"><span className="eyebrow">COMPLETE TUTORIAL</span><h2>How to win Shattered Oath</h2><p className="modal-lead">Two teams alternate cards and d20 rolls. Eliminate the opposing team, or hold more total HP after turn 30.</p><section className="tutorial-section"><h3><Users size={20}/> Setup</h3><div className="tutorial-steps"><article><b>1</b><div><strong>Choose a class</strong><p>Every 13-card deck contains 3 class specials, 5 common actions, and 5 no-effect common cards. Special cards harm their owner or team on failure; common cards never cause a failure penalty.</p></div></article><article><b>2</b><div><strong>Join and ready up</strong><p>Each browser controls one player session. Everyone shares one room and must press Ready before the battle starts.</p></div></article></div></section><section className="tutorial-section"><h3><Dices size={20}/> Playing a turn</h3><div className="tutorial-steps"><article><b>1</b><div><strong>Choose a highlighted card</strong><p>Attacks target enemies; Heal and Guard may target allies. No-effect cards change no HP, shield, buffs, or turn order, but playing one cycles it into the discard pile.</p></div></article><article><b>2</b><div><strong>Beat the random target</strong><p>The server reveals a fresh target from 8 to 16 every turn. It is independent of every previous roll. Only active buffs and the acting character's matching passive modify the d20.</p></div></article><article><b>3</b><div><strong>Play or skip</strong><p>A played card enters the discard pile and draws a replacement. Manual Skip and automatic timeout preserve the exact hand, draw pile, and discard pile.</p></div></article></div></section><section className="tutorial-section"><h3><Zap size={20}/> Failure and events</h3><p>Special cards cause self-damage or team damage when their d20 check fails; common cards do not. A player at 0 HP cannot act. Highlighted world-event turns occur every 5 turns and are recorded in Battle History.</p></section><section className="tutorial-section warning-section"><h3><Clock3 size={20}/> Synchronized turns</h3><p>Every client uses the server clock for the same 30-second deadline. At zero, the server immediately passes the turn without a timeout popup and preserves the player's hand. Eliminating a team wins immediately; after turn 30, total HP decides the winner.</p></section></div> :
