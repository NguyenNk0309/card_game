...
function HandCardContents({ card, pityCostOverride }: { card: ActionCard; pityCostOverride?: number }) {
  return <><PityCostBadge card={card} costOverride={pityCostOverride}/><span className={`special-skill-banner ${card.unique ? "" : "common-card-banner"}`}>{card.unique ? <><Crown size={12}/> SPECIAL</> : "COMMON"}</span><div className={`card-sigil effect-${card.effect}`}><CardEffectIcon card={card}/></div><span>{card.unique ? "Special card" : card.effect === "none" ? "No-effect card" : "Common action"}</span><strong>{card.name}</strong><CardDescription card={card}/><CardOutcomeLines card={card}/></>;
}

function PublicDeck({ player, localPlayer }: { player: PlayerSession; localPlayer?: PlayerSession }) {
  return <div className="public-character-deck"><div className="public-deck-heading"><div><span className="eyebrow">PUBLIC CHARACTER DECK · <b className={`player-name-highlight ${playerRelationClass(player, localPlayer)}`}>{player.displayName}</b></span><strong>{player.hero.name} · 10 cards</strong></div></div><div>{player.skillDeck.map((card) => <article key={card.id} className={`public-deck-card card-with-header effect-${card.effect} ${card.unique ? "public-special-card" : ""}`} style={{ "--hero-color": player.hero.color } as React.CSSProperties}><PityCostBadge card={card}/><span className={`unique-card-banner ${card.unique ? "" : "common-card-banner"}`}>{card.unique ? <><Crown size={13}/> SPECIAL</> : "COMMON"}</span><div className={`card-sigil effect-${card.effect}`}><CardEffectIcon card={card}/></div><div><span>{card.unique ? "Special" : "Common"}</span><strong>{card.name}</strong><CardDescription card={card}/><CardOutcomeLines card={card}/></div></article>)}</div></div>;
}
...
function ViewpointPlayerText({ text = "", players, localPlayer, involvedPlayerIds = [], emphasizedPlayerIds = [], onInspect }: { text?: string; players: PlayerSession[]; localPlayer?: PlayerSession; involvedPlayerIds?: string[]; emphasizedPlayerIds?: string[]; onInspect?: (id: string) => void }) {
  const formatted = formatViewpointText(text, players, localPlayer?.id, { involvedPlayerIds, emphasizedPlayerIds });
  return <HighlightPlayerNames text={formatted} players={players} localPlayer={localPlayer} onInspect={onInspect}/>;
}
...
function LocalTurnActionPanel({ outcome, players, localPlayer, onContinue }: { outcome: GameOutcome; players: PlayerSession[]; localPlayer?: PlayerSession; onContinue: () => void }) {
  ...
  return <div className={`resolution-content local-turn-action-content ${discarded ? "discard" : "skip"}`}><div className="resolution-hero world">{discarded ? <Archive size={34}/> : <Hourglass size={34}/>}</div><span className="eyebrow">{presentation.category}</span><h2><HighlightPlayerNames text={presentation.title} players={players} localPlayer={localPlayer}/></h2><p className="modal-lead"><HighlightPlayerNames text={presentation.detail} players={players} localPlayer={localPlayer}/></p><button className="primary-button continue-button" onClick={onContinue}>Continue <ChevronRight size={17}/></button></div>;
}
...
function RunStatus({ completedPhases, secondsLeft, worldEvents, pendingPhase, onOpenWorldEvents }: { completedPhases: number; secondsLeft: number; worldEvents: SyncedGameState["worldEventHistory"]; pendingPhase?: number; onOpenWorldEvents: () => void }) {
  ...
  })}</div><div className={`turn-clock ${pendingPhase ? "event-paused" : secondsLeft <= 10 ? "urgent" : ""}`}><Clock3 size={14}/> {pendingPhase ? "Event choice" : `${secondsLeft} seconds`}</div><button type="button" className="icon-button phase-world-events-button" onClick={onOpenWorldEvents} aria-label="View all World Events" title="View all World Events"><Zap size={17}/></button></nav>;
}
...
function RoomGame({ roomId, onRoomUnavailable, onReturnHome }: { roomId: string; onRoomUnavailable: (message: string) => void; onReturnHome: () => void }) {
  ...
  const [expandedPanel, setExpandedPanel] = useState<"history" | "turns" | "world-events" | null>(null);
  ...
  const currentBattlePhase = Math.min(30, (game?.completedPhases ?? 0) + 1);
      ...
      <div className="game-layout">{mobileParty && <button className="mobile-rail-backdrop" onClick={() => setMobileParty(false)} aria-label="Close player list"/>}<div className={mobileParty ? "mobile-rail open" : "mobile-rail"}><button className="mobile-close icon-button" onClick={() => setMobileParty(false)}><X size={17}/></button><PartyRail players={players} game={game} localSessionId={localPlayer ? sessionId : ""} onInspectPlayer={inspectPlayer}/></div><PartyRail players={players} game={game} localSessionId={localPlayer ? sessionId : ""} onInspectPlayer={inspectPlayer}/>
        <section className="world-stage combat-stage"><div className="realm-meta"><div><span className="eyebrow">TEAM BATTLE ARENA · 30-PHASE MATCH</span><h1>Eliminate the opposing team</h1></div></div>
          ...
          <section className="hand-zone private-hand-zone"><div className="hand-heading"><div><span className="eyebrow">{localPlayer ? "YOUR PRIVATE HAND" : "PRIVATE HAND"}</span><strong>{activePlayer?.id === sessionId ? "Choose a card and target, then roll." : "Plan while the current player acts."}</strong></div>{localPlayer && <div className="pile-review-actions"><span><Hand size={16}/> Hand ({localState?.hand.length ?? 0})</span><button onClick={() => setDeckReview("draw")}><Layers size={16}/> Draw pile ({localState?.drawPile.length ?? 0})</button><button onClick={() => setDeckReview("discard")}><Archive size={16}/> Discard ({localState?.discardPile.length ?? 0})</button><button onClick={() => setDeckReview("graveyard")}><Skull size={16}/> Graveyard ({localState?.graveyard?.length ?? 0})</button></div>}</div>
            {localState && <div className="active-effects-strip"><b>ACTIVE EFFECTS</b>{localStatusPresentations.map((status) => <span className={status.negative ? "effect-penalty" : status.kind === "attackBuff" ? "effect-attack" : status.kind === "revive" ? "effect-heal" : "effect-support"} title={status.tooltip} key={status.kind}>{status.label.replace(/^Your /, "")} {status.displayValue}{status.duration && !status.displayValue.endsWith(status.duration) ? ` · ${status.duration}` : ""}</span>)}{!localStatusPresentations.length && <span>No active effects</span>}</div>}
