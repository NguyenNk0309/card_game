...
import { AudioLines, Check, ChevronRight, CircleHelp, Clock3, Crown, Dices, Eye, Flame, Hand, Heart, History, LogOut, Octagon, RefreshCw, Shield, SkipForward, Skull, Sparkles, Swords, Target, Trash2, Users, Volume2, X, Zap } from "lucide-react";
...
export default function GameApp() {
  ...
  const passiveDiceBonus = activePlayer && activeCard && activeState ? getPassiveDiceBonus(activePlayer, activeCard, activeState) : 0;
  ...
  return <main className="game-shell arena-focus"><div className="grain"/>
      ...
      <div className="game-layout"><div className={mobileParty ? "mobile-rail open" : "mobile-rail"}><button className="mobile-close icon-button" onClick={() => setMobileParty(false)}><X size={17}/></button><PartyRail players={players} activePlayerId={activePlayer?.id ?? ""} game={game} localSessionId={localPlayer ? sessionId : ""} onRemovePlayer={removePlayer}/></div><PartyRail players={players} activePlayerId={activePlayer?.id ?? ""} game={game} localSessionId={localPlayer ? sessionId : ""} onRemovePlayer={removePlayer}/>
        <section className="world-stage combat-stage"><div className="realm-meta"><div><span className="eyebrow">TEAM BATTLE ARENA · 30-TURN MATCH</span><h1>Eliminate the opposing team</h1></div><span className="seed-button">d20 target: {adventure.target}</span></div>
          ...
          <section className="hand-zone"><div className="hand-heading"><div><span className="eyebrow">{activePlayer?.displayName?.toUpperCase()}'S HAND</span><strong>Choose a card, select a target when required, then roll the die</strong></div><span><Hand size={14}/> {activeState?.hand.length ?? 0} in hand · {activeState?.drawPile.length ?? 0} draw pile · <Trash2 size={14}/> {activeState?.discardPile.length ?? 0} discard pile</span></div>
            ...
            <div className="action-hand four-cards">{activeHand.map((card) => <button className={`action-card ${card.unique ? "hero-special-card" : "common-action-card"} ${card.effect === "none" ? "no-effect-card" : ""} ${selectedCard === card.id ? "selected" : ""}`} aria-pressed={selectedCard === card.id} style={{ "--hero-color": activePlayer?.hero.color } as React.CSSProperties} key={card.id} onClick={() => setSelectedCard(card.id)} disabled={activePlayer?.id !== sessionId || runComplete || (activeState?.hp ?? 0) <= 0}>{card.unique && <span className="special-skill-banner"><Crown size={12}/> {activePlayer?.hero.className.toUpperCase()} SKILL</span>}<div className={`card-sigil ${card.type.toLowerCase()}`}>{card.type === "Might" ? <Swords size={18}/> : card.type === "Wit" ? <Eye size={18}/> : <Sparkles size={18}/>}</div><span>{card.unique ? "Special card" : card.effect === "none" ? "No-effect card" : "Common action"} · {typeName[card.type]}</span><strong>{card.name}</strong><p>{card.description}</p><small>{getCardEffectLabel(card)} · {getCardTargetLabel(card)}</small><p className="card-impact">{describeCardImpact(card)}</p></button>)}</div>
          </section>
        </section>
    ...
    {modalOpen && <div className="modal-backdrop" role="dialog" aria-modal="true"><section className={`modal-card ${showGuide ? "tutorial-modal" : ""} ${(showOutcome || showWorldEvent) && !showGuide ? "resolution-card" : ""}`}><button className="modal-close icon-button" onClick={closeModal} aria-label="Close"><X size={18}/></button>
      {showGuide ? <div className="tutorial-scroll"><span className="eyebrow">COMPLETE TUTORIAL</span><h2>How to win Shattered Oath</h2><p className="modal-lead">Two teams alternate cards and d20 rolls. Eliminate the opposing team, or hold more total HP after turn 30.</p><section className="tutorial-section"><h3><Users size={20}/> Setup</h3><div className="tutorial-steps"><article><b>1</b><div><strong>Choose a class</strong><p>Every 13-card deck contains 3 class specials, 5 common actions, and 5 no-effect common cards. Every special card harms its owner or team when its roll fails.</p></div></article><article><b>2</b><div><strong>Join and ready up</strong><p>Each browser controls one player session. Everyone shares one room and must press Ready before the battle starts.</p></div></article></div></section><section className="tutorial-section"><h3><Dices size={20}/> Playing a turn</h3><div className="tutorial-steps"><article><b>1</b><div><strong>Choose a highlighted card</strong><p>Attacks target enemies; Heal and Guard may target allies. No-effect cards change no HP, shield, buffs, or turn order, but playing one cycles it into the discard pile.</p></div></article><article><b>2</b><div><strong>Roll without a card bonus</strong><p>Cards have no built-in d20 bonus. Only an active card buff or the acting character's matching class passive can increase the roll; enemy effects can reduce it.</p></div></article><article><b>3</b><div><strong>Play or skip</strong><p>A played card enters the discard pile and draws a replacement. Manual Skip and automatic timeout preserve the exact hand, draw pile, and discard pile.</p></div></article></div></section><section className="tutorial-section"><h3><Zap size={20}/> Failure and events</h3><p>All special cards cause self-damage or team damage when their d20 check fails. A player at 0 HP cannot act. An increasingly powerful world event occurs every 5 turns.</p></section><section className="tutorial-section warning-section"><h3><Clock3 size={20}/> Synchronized turns</h3><p>Every client uses the server clock for the same 30-second deadline. At zero, the server immediately passes the turn without a timeout popup and preserves the player's hand. Eliminating a team wins immediately; after turn 30, total HP decides the winner.</p></section></div> :
