...
function CardZoneVfx({ motion }: { motion: { id: number; slotIndex: number; drawn?: ActionCard; discarded?: ActionCard } }) {
  const [slotStyle, setSlotStyle] = useState<React.CSSProperties | null>(null);
  useLayoutEffect(() => {
    const slot = document.querySelector<HTMLElement>(`.action-hand [data-hand-slot="${motion.slotIndex}"]`);
    if (!slot) return;
    const placeOverSlot = () => {
  ...
  return <div className="card-zone-vfx" aria-hidden="true">{motion.discarded && <div className={`card-motion hand-to-discard effect-${motion.discarded.effect}`} style={slotStyle}><span>Leaving hand</span><div className="card-sigil"><CardEffectIcon card={motion.discarded}/></div><strong>{motion.discarded.name}</strong><small>Moved to discard</small></div>}{motion.drawn && <div className={`card-motion draw-to-hand effect-${motion.drawn.effect}`} style={slotStyle}><span>Replacement card</span><div className="card-sigil"><CardEffectIcon card={motion.drawn}/></div><strong>{motion.drawn.name}</strong><small>Drawn into this slot</small></div>}</div>;
}
...
export default function GameApp() {
  ...
  const manualPanelOpen = showGuide || Boolean(deckReview) || Boolean(expandedPanel) || Boolean(inspectedPlayer) || showRunComplete;
  ...
  useEffect(() => {
    ...
    if (previous?.playerId === localPlayer.id) {
      ...
      const slotIndex = pending?.slotIndex ?? (discardedId ? Math.max(0, previous.hand.indexOf(discardedId)) : Math.max(0, localState.hand.indexOf(drawnId ?? "")));
      const discarded = pending?.discarded ?? (discardedId ? cardCatalog.find((card) => card.id === discardedId) : undefined);
      const replacementId = pending ? localState.hand[slotIndex] : drawnId;
  ...
  return <main className="game-shell arena-focus"><div className="grain"/>{showBattleVfx && vfxCard && <div className={`battle-card-vfx effect-${vfxCard.effect} ${outcome?.success ? "success" : "failure"}`} aria-hidden="true"><i/><i/><i/><div><CardEffectIcon card={vfxCard}/><strong>{vfxCard.name}</strong></div></div>}{cardZoneMotion && !modalOpen && <CardZoneVfx key={cardZoneMotion.id} motion={cardZoneMotion}/>} {phase === "game" && localPlayer && <button className="discard-card-button" onClick={discardCard} disabled={activePlayer?.id !== sessionId || status !== "connected" || runComplete || rolling || !activeCard || (activeState?.hp ?? 0) <= 0}><Trash2 size={19}/><span>Discard selected card</span><small>Draw replacement · End turn</small></button>}{showGuide && <DetailedGuide onClose={() => setShowGuide(false)}/>}
    ...
    {modalOpen && <div className="modal-backdrop" role="dialog" aria-modal="true" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }}><section className={`modal-card ${showGuide ? "tutorial-modal" : ""} ${deckReview || expandedPanel || (inspectedPlayer && inspectedView === "deck") ? "wide-modal" : ""} ${(showOutcome || showTurnSummary || showWorldEvent) && !showGuide && !deckReview && !expandedPanel && !inspectedPlayer ? "resolution-card" : ""}`}><button className="modal-close icon-button" onClick={closeModal} aria-label="Close"><X size={18}/></button>
