...
function LocalTurnActionPanel({ outcome, players, localPlayer, cardNames, onInspectCard, onContinue }: { outcome: GameOutcome; players: PlayerSession[]; localPlayer?: PlayerSession; cardNames: readonly string[]; onInspectCard: (name: string) => void; onContinue: () => void }) {
  ...
  return <div className={`resolution-content local-turn-action-content ${discarded ? "discard" : "skip"}`}><div className="resolution-hero world">{discarded ? <Archive size={34}/> : <Hourglass size={34}/>}</div><h2><HighlightInteractiveNames text={presentation.title} cardNames={cardNames} players={players} localPlayer={localPlayer} onInspectCard={onInspectCard} useActualNames/></h2><p className="modal-lead"><HighlightInteractiveNames text={presentation.detail} cardNames={cardNames} players={players} localPlayer={localPlayer} onInspectCard={onInspectCard} useActualNames/></p><button className="primary-button continue-button" onClick={onContinue}>Continue <ChevronRight size={17}/></button></div>;
}
