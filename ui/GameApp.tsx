  ...
  return <div className="guide-update-backdrop" onClick={onClose}>
    <section className="guide-update-panel" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="guide-update-grid">
        ...
        <article><strong>6 · World event system</strong><p>Every fifth phase triggers a random World Event that affects every living player independently.</p></article>
...
function RunStatus({ completedPhases, secondsLeft, worldEvents }: { completedPhases: number; secondsLeft: number; worldEvents: GameHistoryEntry[] }) {
  ...
  return <nav className="run-status" aria-label="Battle status"><div><span className="eyebrow">PHASE</span><strong>{currentPhase} <i>/ 30</i></strong></div><div className="chapter-pips" aria-label="Thirty-phase timeline">{Array.from({ length: 30 }).map((_, index) => {
    ...
    const isWorldEvent = phase % 5 === 0;
    const resolvedEvent = isWorldEvent ? worldEvents.find((entry) => entry.phase === phase) : undefined;
    const eventDetail = resolvedEvent?.message ?? `At the end of phase ${phase}, a level ${phase / 5} World Event gives every living player a separate random result: damage, healing, shield loss, an attack bonus, or a volatile HP change.`;
    return <i key={phase} data-turn={phase} title={isWorldEvent ? undefined : `Phase ${phase}`} tabIndex={isWorldEvent ? 0 : undefined} aria-label={isWorldEvent ? `Phase ${phase}, World Event. ${eventDetail}` : `Phase ${phase}`} className={`${phaseClass} ${isWorldEvent ? "world-event-turn" : ""} ${resolvedEvent ? "event-triggered" : ""}`}>{isWorldEvent && <span className="phase-event-tooltip" role="tooltip"><b><Zap size={13}/> Phase {phase} World Event</b><small>{eventDetail}</small></span>}</i>;
  })}</div><div className={`turn-clock ${secondsLeft <= 10 ? "urgent" : ""}`}><Clock3 size={14}/> {secondsLeft} seconds</div></nav>;
}
...
export default function GameApp() {
  ...
  const [dismissedLifeEventIds, setDismissedLifeEventIds] = useState<string[]>([]);
  ...
  const activeAutoPanel = manualPanelOpen ? null : showOutcome ? "outcome" : showTurnSummary ? "summary" : showWorldEvent ? "world" : showLifeEvent ? "life" : null;
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      else if (activeAutoPanel === "world" && game?.worldEvent) setDismissedWorldEventId(game.worldEvent.id);
      else if (activeAutoPanel === "life" && activeLifeEvent) setDismissedLifeEventIds((current) => [...current, activeLifeEvent.id]);
    }, 10000);
  ...
  const closeModal = () => {
    ...
    if (showLifeEvent && activeLifeEvent) setDismissedLifeEventIds((current) => [...current, activeLifeEvent.id]);
  ...
  return <main className="game-shell arena-focus"><div className="grain"/>{visibleNotices.length > 0 && <section className="game-notice-stack" aria-live="polite" aria-label="Battle notices">{visibleNotices.map((notice) => <article className={`outcome-toast game-notice ${notice.kind}`} key={notice.id}><GameNoticeIcon kind={notice.kind}/><div><strong>{notice.title}</strong><span><HighlightPlayerNames text={notice.detail} players={players} localPlayer={localPlayer}/></span></div></article>)}</section>}{showBattleVfx && vfxCard && <div className={`battle-card-vfx effect-${vfxCard.effect} ${outcome?.success ? "success" : "failure"}`} aria-hidden="true"><i/><i/><i/><div><CardEffectIcon card={vfxCard}/><strong>{vfxCard.name}</strong></div></div>}{cardZoneMotion && localPlayer && !panelOverlayOpen && <CardZoneVfx key={cardZoneMotion.id} motion={cardZoneMotion} player={localPlayer} playable={activePlayer?.id === sessionId && !runComplete && (localState?.hp ?? 0) > 0}/>} {showGuide && <DetailedGuide onClose={() => setShowGuide(false)}/>}
    ...
    {modalOpen && <div className="modal-backdrop" role="dialog" aria-modal="true" onClick={closeModal}><section className={`modal-card ${showGuide ? "tutorial-modal" : ""} ${deckReview || expandedPanel || inspectedPlayer ? "wide-modal" : ""} ${(showOutcome || showTurnSummary || showLifeEvent || showWorldEvent) && !showGuide && !deckReview && !expandedPanel && !inspectedPlayer && !inspectedCard ? "resolution-card" : ""}`} onClick={(event) => event.stopPropagation()}><button className="modal-close icon-button" onClick={closeModal} aria-label="Close"><X size={18}/></button>
