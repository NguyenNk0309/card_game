...
function HighlightPlayerNames({ text = "", players, localPlayer, onInspect }: { text?: string; players: PlayerSession[]; localPlayer?: PlayerSession; onInspect?: (id: string) => void }) {
  ...
  return <>{parts.map((part, index) => {
    ...
    return onInspect
      ? <button type="button" className={`${className} history-player-link`} title={player.hero.name} aria-label={`View ${playerDisplayName(player, localPlayer?.id)}, ${player.hero.name}`} onClick={() => onInspect(player.id)} key={`${part}-${index}`}>{label}</button>
...
function HistoryEntries({ entries, players, localPlayer, expanded = false, onInspectPlayer, onInspectCard }: { entries: GameHistoryEntry[]; players: PlayerSession[]; localPlayer?: PlayerSession; expanded?: boolean; onInspectPlayer: (id: string) => void; onInspectCard: (name: string) => void }) {
  ...
  const renderEntry = (entry: GameHistoryEntry) => <article className={`history-entry ${entry.kind} ${entry.success ? "success" : "failure"}`} key={entry.id}><span className={entry.kind === "world" ? "history-event-label" : ""}>{entry.kind === "world" && <Zap size={12}/>} {entry.kind === "world" ? expanded ? "WORLD EVENT" : `WORLD EVENT · PHASE ${entry.phase ?? entry.turn}` : `Turn ${entry.turn}`}</span><p><HistoryMessage entry={entry} players={players} localPlayer={localPlayer} onInspectPlayer={onInspectPlayer} onInspectCard={onInspectCard}/></p>{entry.diceRoll != null && (entry.resolution === "roll" && entry.pityCost === 0 ? <div className="history-dice automatic"><span>d20: {entry.diceRoll} · ignored</span><strong>Automatic success · zero-pity card</strong></div> : <div className="history-dice"><span>d20: {entry.diceRoll}</span><span>Active/passive bonus: +{entry.diceBonus ?? 0}</span>{Boolean(entry.dicePenalty) && <span>Penalty: -{entry.dicePenalty}</span>}<strong>Total {entry.diceTotal} / target {entry.diceTarget}</strong></div>)}{entry.resolution === "pity" && <div className="history-pity"><PityIcon size={13}/><span>Guaranteed success · spent {entry.pityCost ?? 0}</span><strong>{entry.pityBefore ?? 0} → {entry.pityAfter ?? 0} pity</strong></div>}</article>;
...
function TurnOrderList({ players, game, order, localPlayer, expanded = false, onInspect }: { players: PlayerSession[]; game: SyncedGameState | null; order: PlayerSession[]; localPlayer?: PlayerSession; expanded?: boolean; onInspect: (id: string) => void }) {
  ...
  return <div className={`turn-queue-list ${expanded ? "expanded-turn-list turn-branch" : ""}`}>{branchPlayers.map((player) => { const status = player.id === activeId ? "current" : acted.has(player.id) ? "passed" : "future"; return <article className={`turn-queue-item ${status}`} aria-label={`${status}: ${playerDisplayName(player, localPlayer?.id)}`} key={player.id}><button className="portrait-button" onClick={() => onInspect(player.id)} aria-label={`View ${playerPossessiveName(player, localPlayer?.id).toLowerCase()} character`}><div className="portrait mini" style={{ "--hero-color": player.hero.color } as React.CSSProperties}>{player.hero.initials}</div></button><div className="turn-player-copy"><strong className={`player-name-highlight ${playerRelationClass(player, localPlayer)}`}>{playerDisplayName(player, localPlayer?.id)}</strong><span className="turn-player-meta">{player.hero.name} · Speed {player.hero.speed} · {teamName[player.hero.team]} · {game?.playerStates[player.id]?.hp ?? player.hero.hp} HP</span></div>{expanded && <i className="turn-branch-node" aria-hidden="true"/>}</article>; })}</div>;
}
...
export default function GameApp() {
  ...
  const isLocalActiveTurn = phase === "game" && !runComplete && activePlayer?.id === sessionId && (localState?.hp ?? 0) > 0;
  ...
  const joinPlayer = (team: TeamId) => {
    ...
    const resolvedHeroName = selectedHeroName === RANDOM_CHARACTER_SELECTION
      ? characterOptions[Math.floor(Math.random() * characterOptions.length)]?.hero.name
      : selectedHeroName;
    if (!resolvedHeroName) return setLobbyError("No character is available. Please try again.");
    const session = createPlayerSession(name, team === "veil" ? 0 : 1, resolvedHeroName, sessionId);
