...
function HighlightPlayerNames({ text = "", players, localPlayer, onInspect }: { text?: string; players: PlayerSession[]; localPlayer?: PlayerSession; onInspect?: (id: string) => void }) {
  ...
  return <>{parts.map((part, index) => {
    const player = playerByName.get(part.toLocaleLowerCase());
    if (!player) return part;
    const className = `inline-player-name ${playerRelationClass(player, localPlayer)}`;
    return onInspect
      ? <button type="button" className={`${className} history-player-link`} title={player.hero.name} aria-label={`View ${player.displayName}, ${player.hero.name}`} onClick={() => onInspect(player.id)} key={`${part}-${index}`}>{part}</button>
