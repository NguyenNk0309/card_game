...
import type { ActionCard, GameHistoryEntry, GameNotice, PlayerLifeEvent, PlayerSession, SyncedGameState, TeamId, WorldEventOutcome } from "@/shared/types";
...
export default function GameApp() {
  ...
  const deferWorldLifeForLocalTurn = Boolean(activeLifeEvent?.source === "world-event" && isLocalActiveTurn);
  const showLifeEvent = Boolean(activeLifeEvent && !deferWorldLifeForLocalTurn);
  ...
  const showWorldEvent = Boolean(queuedWorldEvent && !isLocalActiveTurn && !worldEventBlocking);
  const showWorldEventNotice = Boolean(queuedWorldEvent && isLocalActiveTurn && !worldEventBlocking);
  ...
  const activeAutoPanel = worldEventBlocking || manualPanelOpen ? null : showOutcome ? "outcome" : showTurnSummary ? "summary" : showWorldEvent ? "world" : showLifeEvent ? "life" : showRunComplete ? "battle" : null;
  ...
  useEffect(() => {
    ...
    if (game.worldEvent) enqueueOnce({ id: `world:${game.worldEvent.id}`, kind: "world", event: game.worldEvent });
    for (const lifeEvent of lifeEvents.filter((event) => event.source === "world-event")) {
      enqueueOnce({ id: `life:${lifeEvent.id}`, kind: "life", lifeEvent });
    }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      else if (activeAutoPanel === "life" && queuedPresentation?.kind === "life") {
  ...
  return <main className="game-shell arena-focus"><div className="grain"/>{visibleNotices.length > 0 && <section className="game-notice-stack" aria-live="polite" aria-label="Battle notices">{visibleNotices.map((notice) => <article className={`outcome-toast game-notice ${notice.kind}`} key={notice.id}><GameNoticeIcon kind={notice.kind}/><div><strong>{notice.title}</strong><span><HighlightPlayerNames text={notice.detail} players={players} localPlayer={localPlayer}/></span></div></article>)}</section>}{showWorldEventNotice && queuedWorldEvent && <aside className={`world-event-nonblocking-notice intensity-${queuedWorldEvent.intensity.toLowerCase()}`} aria-live="polite"><Zap size={20}/><div><span>WORLD EVENT · PHASE {queuedWorldEvent.phase}</span><strong>{queuedWorldEvent.title}</strong><small>Your turn remains active. Full details are preserved in Battle History.</small></div><button onClick={() => { dismissPresentation(); setExpandedPanel("history"); }}>View history</button><button aria-label="Dismiss World Event notice" onClick={() => dismissPresentation()}><X size={15}/></button></aside>}{showBattleVfx && vfxCard && <div className={`battle-card-vfx effect-${vfxCard.effect} ${outcome?.success ? "success" : "failure"}`} aria-hidden="true"><i/><i/><i/><div><CardEffectIcon card={vfxCard}/><strong>{vfxCard.name}</strong></div></div>}{cardZoneMotion && localPlayer && !panelOverlayOpen && <CardZoneVfx key={cardZoneMotion.id} motion={cardZoneMotion} player={localPlayer} playable={activePlayer?.id === sessionId && !runComplete && (localState?.hp ?? 0) > 0}/>} {showGuide && <DetailedGuide onClose={() => setShowGuide(false)}/>} {pendingWorldEvent && <ShatteredTributeChoicePanel pendingEvent={pendingWorldEvent} players={players} localPlayer={localPlayer} localState={localState} handCards={localHand} serverTimeOffsetMs={serverTimeOffsetMs} connectionError={roomError} onSubmit={submitWorldEventChoice}/>}
    ...
    {modalOpen && <div className="modal-backdrop" role={showWorldEvent ? undefined : "dialog"} aria-modal={showWorldEvent ? undefined : true} onClick={showWorldEvent ? undefined : closeModal}><section className={`modal-card ${showGuide ? "tutorial-modal" : ""} ${deckReview || expandedPanel || inspectedPlayer ? "wide-modal" : ""} ${(showOutcome || showTurnSummary || showLifeEvent || showWorldEvent) && !showGuide && !deckReview && !expandedPanel && !inspectedPlayer && !inspectedCard ? "resolution-card" : ""}`} onClick={(event) => event.stopPropagation()}>{!showWorldEvent && <button className="modal-close icon-button" onClick={closeModal} aria-label="Close"><X size={18}/></button>}
      ...
      expandedPanel === "history" ? <div className="expanded-panel-content"><span className="eyebrow">EXPANDED BATTLE HISTORY</span><h2>Every action and World Event</h2><HistoryEntries entries={game?.history ?? []} players={players} localPlayer={localPlayer} expanded onInspectPlayer={inspectPlayer} onInspectCard={inspectCard}/>{Boolean(game?.worldEventHistory.length) && <section className="world-event-history-archive"><h3><Zap size={17}/> Resolved World Event details</h3>{[...(game?.worldEventHistory ?? [])].reverse().map((event) => { const localResult = event.results.find((result) => result.playerId === localPlayer?.id); return <article className={`intensity-${event.intensity.toLowerCase()}`} key={event.id}><header><span>Phase {event.phase} · Level {event.level} · {event.intensity}</span><strong>{event.title}</strong></header><p>{event.fullDescription}</p>{localResult && <div><b>Your private result</b><span>{localResult.privateSummary || localResult.publicSummary}</span></div>}<ul>{event.teamSummaries.map((summary) => <li key={summary.team}><b>{teamName[summary.team]}</b> {summary.summary}</li>)}</ul></article>; })}</section>}</div> :
      ...
      showWorldEvent && queuedWorldEvent ? <ResolvedWorldEventPanel event={queuedWorldEvent} localPlayerId={localPlayer?.id} onContinue={() => dismissPresentation()} onViewHistory={() => { dismissPresentation(); setExpandedPanel("history"); }}/> :
