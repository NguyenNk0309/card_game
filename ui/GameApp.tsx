...
function HandCardContents({ card, player }: { card: ActionCard; player: PlayerSession }) {
  return <>{card.unique && <span className="special-skill-banner"><Crown size={12}/> {player.hero.className.toUpperCase()} SKILL</span>}<div className={`card-sigil effect-${card.effect}`}><CardEffectIcon card={card}/></div><span>{card.unique ? "Special card" : card.effect === "none" ? "No-effect card" : "Common action"} · {typeName[card.type]}</span><strong>{card.name}</strong><p><EffectText text={card.description} card={card}/></p><small>{getCardEffectLabel(card)} · {getCardTargetLabel(card)}</small><CardOutcomeLines card={card}/></>;
}

function PublicDeck({ player, localPlayer }: { player: PlayerSession; localPlayer?: PlayerSession }) {
  return <div className="public-character-deck"><div className="public-deck-heading"><div><span className="eyebrow">PUBLIC CHARACTER DECK · <b className={`player-name-highlight ${playerRelationClass(player, localPlayer)}`}>{player.displayName}</b></span><strong>{player.hero.name} · 10 cards</strong></div></div><div>{player.skillDeck.map((card) => <article key={card.id} className={`public-deck-card effect-${card.effect} ${card.unique ? "public-special-card" : ""}`} style={{ "--hero-color": player.hero.color } as React.CSSProperties}>{card.unique && <span className="unique-card-banner"><Crown size={13}/> SPECIAL · {player.hero.name}</span>}<div className={`card-sigil effect-${card.effect}`}><CardEffectIcon card={card}/></div><div><span>{card.unique ? "Special" : "Common"} · {card.type}</span><strong>{card.name}</strong><small>{getCardEffectLabel(card)} · {getCardTargetLabel(card)}</small><p><EffectText text={card.description} card={card}/></p><CardOutcomeLines card={card}/></div></article>)}</div></div>;
}
  ...
  return <div className="guide-update-backdrop" onClick={onClose}>
    <section className="guide-update-panel" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="guide-update-grid">
        ...
        <article><strong>4 · Play, discard, or skip</strong><p>Playing a selected card rolls against the current d20 target and sends that card through its normal cycle. Manual Discard also ends the turn. Skip and Discard require confirmation. Skip and timeout preserve the exact hand and every card zone.</p></article>
