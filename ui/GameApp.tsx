...
function TurnOrderList({ players, game, order, expanded = false, onInspect }: { players: PlayerSession[]; game: SyncedGameState | null; order: PlayerSession[]; expanded?: boolean; onInspect: (id: string) => void }) {
  ...
  return <div className={`turn-queue-list ${expanded ? "expanded-turn-list turn-branch" : ""}`}>{branchPlayers.map((player, index) => { const status = player.id === activeId ? "current" : acted.has(player.id) ? "passed" : "future"; return <article className={`turn-queue-item ${status}`} key={player.id}><b>{status === "current" ? "ACTIVE" : status === "passed" ? "PASSED" : "PENDING"}</b><button className="portrait-button" onClick={() => onInspect(player.id)} aria-label={`View ${player.displayName}'s character`}><div className="portrait mini" style={{ "--hero-color": player.hero.color } as React.CSSProperties}>{player.hero.initials}</div></button><div><strong className="player-name-highlight">{player.displayName}</strong><span>{player.hero.name} · Speed {player.hero.speed} · {teamName[player.hero.team]} · {game?.playerStates[player.id]?.hp ?? player.hero.hp} HP</span></div>{expanded && <i className="turn-branch-node" aria-hidden="true"/>}</article>; })}</div>;
}
...
function CardZoneVfx({ motion }: { motion: { id: number; drawn?: ActionCard; discarded?: ActionCard } }) {
  return <div className="card-zone-vfx" aria-hidden="true">{motion.drawn && <div className={`card-motion draw-to-hand effect-${motion.drawn.effect}`}><span>Draw pile</span><div className="card-sigil"><CardEffectIcon card={motion.drawn}/></div><strong>{motion.drawn.name}</strong><small>Drawn to hand</small></div>}{motion.discarded && <div className={`card-motion hand-to-discard effect-${motion.discarded.effect}`}><span>Hand</span><div className="card-sigil"><CardEffectIcon card={motion.discarded}/></div><strong>{motion.discarded.name}</strong><small>Moved to discard</small></div>}</div>;
}
...
export default function GameApp() {
  ...
  useEffect(() => {
    if (!cardZoneMotion) return;
    const timer = window.setTimeout(() => setCardZoneMotion(null), 1350);
    return () => window.clearTimeout(timer);
  }, [cardZoneMotion]);
  ...
  return <main className="game-shell arena-focus"><div className="grain"/>{showBattleVfx && vfxCard && <div className={`battle-card-vfx effect-${vfxCard.effect} ${outcome?.success ? "success" : "failure"}`} aria-hidden="true"><i/><i/><i/><div><CardEffectIcon card={vfxCard}/><strong>{vfxCard.name}</strong></div></div>}{cardZoneMotion && <CardZoneVfx key={cardZoneMotion.id} motion={cardZoneMotion}/>} {phase === "game" && localPlayer && <button className="discard-card-button" onClick={discardCard} disabled={activePlayer?.id !== sessionId || status !== "connected" || runComplete || rolling || !activeCard || (activeState?.hp ?? 0) <= 0}><Trash2 size={19}/><span>Discard selected card</span><small>Draw replacement · End turn</small></button>}{inspectedPlayer && <aside className="inspected-deck-overlay"><PublicDeck player={inspectedPlayer}/></aside>}{showGuide && <DetailedGuide onClose={() => setShowGuide(false)}/>}
    ...
    {modalOpen && <div className="modal-backdrop" role="dialog" aria-modal="true"><section className={`modal-card ${showGuide ? "tutorial-modal" : ""} ${deckReview || expandedPanel ? "wide-modal" : ""} ${(showOutcome || showTurnSummary || showWorldEvent) && !showGuide && !deckReview && !expandedPanel && !inspectedPlayer ? "resolution-card" : ""}`}><button className="modal-close icon-button" onClick={closeModal} aria-label="Close"><X size={18}/></button>
      ...
      showTurnSummary && outcome ? <div className="resolution-content"><div className={`resolution-hero ${outcome.success ? "success" : "failure"}`}>{outcome.success ? <Check size={34}/> : <Skull size={34}/>}</div><span className="eyebrow">TURN SUMMARY</span><h2>{outcome.label}</h2><p className="modal-lead">{outcome.detail}</p></div> :
