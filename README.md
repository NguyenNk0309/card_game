...
13. Before phase 3, Shattered Tribute pauses normal turns for up to 60 seconds while every living player privately chooses two owned common cards from their hand, draw pile, or discard pile to move permanently to graveyard. Special and borrowed cards cannot be chosen. Each selected hand card draws one replacement into its vacated slot using the normal empty-draw recycling rule; selections from draw or discard do not add cards to hand. Missing choices are resolved by the server.
